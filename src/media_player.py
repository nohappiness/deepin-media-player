...
from mplayer.player import ASCEPT_16X10_STATE, ASCEPT_1_85X1_STATE, ASCEPT_2_35X1_STATE,\
    ASCEPT_FULL_STATE, ASCEPT_DEFULAT
...
class MediaPlayer(object):
    ...
    def __init_dbus_id(self):  # 初始化DBUS ID 唯一值.
        ...
        for num in dbus_id_list:
            number = int(num) + 65
            if number in range(ord('a'), ord('z')+1) or number in range(ord('A'), ord('Z')+1):
                dbus_id += "." + chr(number)
            else:
    ...
    def scan_end_event(self, scan_dir, summary):
        if self.run_check:
            self.next()
            self.run_check = False
    ...
    def top_toolbar_common_button_clicked(self):
        if self.concise_check is True or self.fullscreen_check:
    ...
    def open_url_dialog(self, check, clear=True):
        ...
        open_url_win.ok_btn.connect("clicked",
                                    self.__url_text_key_press_event, None, open_url_win, check, clear,
                                    open_url_win.url_text)
        ...
        if value == "Return":
            ...
            if url.startswith(("http:", "mms:")):  # 判断是否为http连接.
                if url.startswith("http://v.youku.com"):  # 添加优酷转换.
                    self.__url_add_to_playlist(text=text, site="youku", bit="优酷视频", check=check, clear=clear)
                elif url.find("pps") != -1:
                    self.__url_add_to_playlist(text=text, site="pps", bit="pps视频", check=check, clear=clear)
                else:
    ...
    def __url_add_to_playlist(self, text, site, bit, check, clear):
        flvcd = YouToFlvcd(site)
