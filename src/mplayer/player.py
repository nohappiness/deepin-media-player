...
class LDMP(gobject.GObject):
    ...
    def play(self):
        ...
        if self.player.softvol:
            ...
            if self.player.volume_gain != 0:
                self.command.append("-af-add")
                self.command.append("volume=%lf:0" % self.player.volume_gain)
            self.command.append("-softvol")
        ...
        if self.player.subtitle_codepage and len(self.player.subtitle_codepage):
        ...
        # if filename:
            ...
            if self.player.tv_width > 0:
                ...
                self.command.append("%d" % self.player.tv_width)
            if self.player.tv_height > 0:
                self.command.append("-tv:height")
                self.command.append("%d" % self.player.tv_height)
            if self.player.tv_fps > 0:
                self.command.append("-tv:fps")
                self.command.append("%d" % self.player.tv_fps)
            self.command.append("-nocache")
    ...
    def sub_up_scale(self):
        self.player.subtitle_scale += 0.1
        self.sub_scale(self.player.subtitle_scale)
    ...
    def fseek(self, seek_num):
        ...
        if self.player.state == STARTING_STATE or self.player.state == PAUSE_STATE:
            ...
            # FIXME: change to correct exception.
            except Exception, e:
    ...
    #         path = self.path
    #     os.system(
    #         "mplayer -ss 1 -noframedrop -nosound -vo %s:outdir=%s -frames 1 %s >/dev/null 2>&1" % (type, outdir, path))
    ...
    def player_thread_reader(self, source, condition):
        ...
        try:
            cache = source.readline()
        # FIXME: add correct exception.
        except:
            return False # 白屏BUG，所以返回假.
            # '''
        # if cache == gobject.IO_STATUS_ERROR:
        #     if DEBUG:
        #         print "GIO IO Error:", cache
        #     return True
        # else:
        #     if DEBUG:
        #         if cache.find("ANS") == None:
        #             print cache
        # '''
        if cache:
            if cache.startswith("Cache fill"):
                print "Cache fill:", cache
                # percent = cache.replace("Cache fill:")
            #    
            if cache.startswith("AO:"):
                self.cmd("get_property switch_audio\n")

            if cache.startswith("VO:"):
                # VO: [xv] 1280x720 => 1280x720 Planar YV12
                split_text = cache.split(" ")
                # 获取视频的宽和高.
            ...
            if cache.startswith("Video: no video"):
                self.player.video_width = 0
                self.player.video_height = 0
                self.emit("screen-changed", self.player.video_width, self.player.video_height)
                #
            if cache.startswith("ANS_TIME_POSITION"):
                pos = float(cache.replace("ANS_TIME_POSITION=", "").split("\n")[0])
                ...
                if old_position != self.player.position:
                    ...
                    if pos:
                        try:
                            ...
                            # '''
                            #
            if cache.startswith("ID_START_TIME"):
                self.player.start_time = float(cache.replace("ID_START_TIME=", "").split("\n")[0])
                # print "ID_START_TIME:", self.player.start_time
            #    
            if cache.startswith("ID_LENGTH"):
                length = float(cache.replace("ID_LENGTH=", "").split("\n")[0])
                self.player.length = length
                # print "ID_LENGTH:", length
                self.emit("get-time-length", length, length_to_time(length))

            #    
            if cache.startswith("ANS_LENGTH"):
                length = float(cache.replace("ANS_LENGTH=", "").split("\n")[0])
                # print "ANS_LENGTH:", length
                self.player.length = length
                self.emit("get-time-length", length, length_to_time(length))
                #
            if cache.startswith("ID_AUDIO_TRACK"):
                self.audio_track = cache.replace("ID_AUDIO_TRACK=", "").split("\n")[0]
                # print "ID_AUDIO_TRACK:", self.audio_track

            if cache.startswith("ANS_switch_audio"):
            # 字幕选择的那个!!
                switch_audio_index = cache.replace("ANS_switch_audio=", "").split("\n")[0]
                self.player.audio_select_index = switch_audio_index

            if cache.startswith("ANS_sub_source"):
                self.player.subtitle_source = cache.replace("ANS_sub_source=", "").split("\n")[0]
            ...
            if cache.startswith("ANS_sub_file"):
                print "ANS_sub_file:", cache.replace("ANS_sub_file", "").split("\n")[0]
                # self.emit()

            if cache.startswith("ANS_sub_demux"):
                print cache.replace("ANS_sub_demux", "").split("\n")[0]
                # self.emit()

            if cache.startswith("DVDNAV_TITLE_IS_MENU"):
            ...
            if cache.startswith("DVDNAV_TITLE_IS_MOVIE"):
            ...
            if cache.startswith("ID_SUBTITLE_ID="):
                # FIXME: change variable name 'text_id'
                text_id = cache.replace("ID_SUBTITLE_ID=", "").split("\n")[0]
                self.player.sub_index += 1  # int(text_id)
                self.player.subtitle.append(text_id)

            if cache.startswith("ID_SID_"):
                text = cache.split("_")[3]
                if text.startswith("NAME="):
                    ...
                    # FIXME: use correct exception.
                        ...
                        name = ""
                    text = name + "（" + lang + "）" + "-" + text_id
                    self.player.subtitle[self.player.sub_index] = text
                    # 发送字幕字符串.
                    self.emit("get-subtitle", text, self.player.sub_index)

            if cache.startswith("ID_FILE_SUB_ID="):
                text_id = cache.replace("ID_FILE_SUB_ID=", "")
                print "ID_FILE_SUB_ID:", text_id

            if cache.startswith("ID_AUDIO_ID="):
                text_id = cache.replace("ID_AUDIO_ID=", "").split("\n")[0]
                self.player.audio_index += 1  # int(text_id)
                self.player.audio_list.append(text_id)

            if cache.startswith("ID_AID_"):
                text = cache.split("\n")[0]
            ...
            if cache.startswith("ID_CHAPTERS="):
                print "ID_CHAPTERS:", cache.replace("ID_CHAPTERS=", "")
                pass

            if cache.startswith("ID_SEEKABLE=") and cache.startswith("ID_SEEKABLE=0"):
                print "ID_SEEKABLE and ID_SEEKABLE=0:", cache
                pass

            if cache.startswith("ID_VIDEO_FORMAT"):
                self.player.video_format = cache.replace("ID_VIDEO_FORMAT=", "").split("\n")[0]
                # print "ID_VIDEO_FORMAT:", self.player.video_format
                # cache.startswith("ID_VIDEO_FORMAT")
                # len("ID_VIDEO_FORMAT=")
                # self.file_info.width == 0 and self.file_info.height == 0

            if cache.startswith("ID_VIDEO_CODEC"):
                self.player.video_codec = cache.replace("ID_VIDEO_CODEC=", "").split("\n")[0]
                # print "ID_VIDEO_CODEC:", self.player.video_codec

            if cache.startswith("ID_VIDEO_FPS"):
                self.player.video_fps = cache.replace("ID_VIDEO_FPS=", "").split("\n")[0].strip()
                # print "ID_VIDEO_FPS:", self.player.video_fps

            if cache.startswith("ID_VIDEO_BITRATE"):
                self.player.video_bitrate = cache.replace("ID_VIDEO_BITRATE=", "").split("\n")[0]
                # print "ID_VIDEO_BITRATE:", self.player.video_bitrate                
            if cache.startswith("ID_AUDIO_FORMAT"):
                self.player.audio_format = cache.replace("ID_AUDIO_FORMAT=", "").split("\n")[0]
                # print 'ID_AUDIO_FORMAT:', self.player.audio_format
            if cache.startswith("ID_AUDIO_CODEC"):
                self.player.audio_codec = cache.replace("ID_AUDIO_CODEC=", "").split("\n")[0]
                # print "ID_AUDIO_CODEC:", self.player.audio_codec

            if cache.startswith("ID_AUDIO_BITRATE"):
                self.player.audio_bitrate = cache.replace("ID_AUDIO_BITRATE=", "").split("\n")[0]
                # print "ID_AUDIO_BITRATE:", self.player.audio_bitrate

            if cache.startswith("ID_AUDIO_RATE"):
                self.player.audio_rate = cache.replace("ID_AUDIO_RATE=", "").split("\n")[0]
                # print "ID_AUDIO_RATE:", self.player.audio_rate

            if cache.startswith("ID_AUDIO_NCH"):
                self.player.audio_nch = cache.replace("ID_AUDIO_NCH=", "").split("\n")[0]
                # print "ID_AUDIO_NCH:", self.player.audio_nch

            #DVD获取信息.
            if cache.startswith("ID_DVD_TITLE_"):
                ...
                index_str = cache.replace(ID_DVD_TITLE, "").split("\n")[0]
                title_index = compile_str.findall(index_str)[0]
                title_length = cache.replace("%s%s%s" % (ID_DVD_TITLE, title_index, LENGTH), "")
                ...
                #title_chapters_number = compile_str.findall(cache
            ...
            if cache.startswith("DVDNAV, switched to title: "):
                self.player.title_index = int(cache.replace("DVDNAV, switched to title: ", "").strip())
                self.emit("dvd-switch-title")

            if cache.startswith("*** screenshot"):
            #... ...
                print cache

            if cache.startswith("Name   : "):
                print "name:", cache

            if cache.startswith("Genre  : "):
                print "Genre:", cache

            if cache.startswith("Title: "):
                print "Title:", cache

            if cache.startswith("Artist: "):
                print "Artist:", cache

            if cache.startswith("Album: "):
                print "Album:", cache

            if cache.startswith("ICY Info"):
                print "ICY info:", cache
                # ... ...

            if cache.startswith("ID_FILENAME"):
                file_name = cache.replace("ID_FILENAME=", "").split("\n")[0] + "\0"
    ...
    def player_thread_reader_error(self, source, condition):  # 错误处理.
        ...
        if self.player.type == TYPE_FILE:
            if cache.startswith("Failed to recognize file format"):
                error_code = ERROR_FILE_FORMAT
        elif self.player.type == TYPE_DVD:
            # if cache.startswith("Couldn't open DVD device"):
            #     error_code = ERROR_DVD_DEVICE
            pass
        elif self.player.type == TYPE_NETWORK:
            if cache.startswith("mplayer: could not connect to socket"):
                error_code = ERROR_SOCKET_CONNECT
            elif (cache.startswith("Couldn't resolve name for AF_INET")
                  or cache.startswith("Couldn't resolve name for AF_INET6")):
                error_code = ERROR_RESOLVE_AF_INET

                # if cache.startswith("X11 error"):
                #     error_code = ERROR_X11

                # if cache.startswith("Failed creating VDPAU decoder"):
                # if (self.player.enable_divx
                # and self.player.vo.startswith("vdpau")):
                # error_code = ERROR_RETRY_WITHOUT_DIVX_VDPAU

        # if cache.startswith("decoding to PIX_FMT_NONE is not supported"):
        #     if self.player.enable_divx:
        #         error_code = ERROR_RETRY_WITHOUT_HARDWARE_CODECS

        # if cache.startswith("The selected video_out device is incompatible with this codec"):
        #     if (self.player.disable_xvmc 
        #         and self.player.startswith("xvmc")):
        #         error_code = ERROR_RETRY_WITHOUT_XVMC

        if cache.startswith("[AO_ALSA] Playback open error: Device or resource busy"):
            error_code = ERROR_RETRY_ALSA_BUSY

        if cache.startswith("Failed to open"):
            if cache.find("mms://") and self.player.type == TYPE_NETWORK:
                error_code = ERROR_RETRY_WITH_MMSHTTP

        if cache.startswith("No stream found to handle url mmshttp://"):
            error_code = ERROR_RETRY_WITH_HTTP

        if (cache.startswith("Server returned 404:File Not Found")
            and self.player.uri.startswith("mmshttp://")):
            error_code = ERROR_RETRY_WITH_HTTP

        if cache.startswith("unknown ASF streaming type") and self.player.uri.startswith("mmshttp://"):
            error_code = ERROR_RETRY_WITH_HTTP

        if cache.startswith("Error while parsing chunk header"):
            error_code = ERROR_RETRY_WITH_HTTP_AND_PLAYLIST

        if cache.startswith("Failed to initiate \"video/X-ASF-PF\" RTP subsession"):
            error_code = ERROR_RETRY_WITH_PLAYLIST

        if cache.startswith("moov atom not found"):
            self.playerretry_on_full_cache = True

        if cache.startswith("MOV: missing header (moov/cmov) chunk"):
            self.playerretry_on_full_cache = True

        if cache.startswith("Seek failed"):
            self.quit()
...
def set_ascept_function(screen_frame, video_aspect):
    ...
    #
    if screen_frame_aspect == video_aspect or video_aspect is None:
...
# 判断目录是否存在，不存在则创建.
    ...
    # scrot image.
    os.system("cd /tmp/buffer/ && mplayer -ss %s -noframedrop -nosound -vo png -frames 1 '%s' >/dev/null 2>&1" % (
        int(scrot_pos), play_path))
